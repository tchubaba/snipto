    ...
    <div class="max-w-3xl mx-auto py-12 px-6 text-gray-900 dark:text-gray-100 leading-relaxed">
        ...
        <div class="mb-12">
            <ul class="space-y-2 list-disc list-inside text-lg">
                ...
                <li><a href="#iv" class="text-indigo-600 dark:text-indigo-400 hover:underline">{!! __('What’s an initialization vector (IV)?') !!}</a></li>
            </ul>
        </div>
        ...
        <div class="space-y-10">
            ...
            <div id="encryption-key" class="scroll-mt-13">
                ...
                <p class="mt-2">
                    {!! __('The encryption key is included in the URL after the <code>#</code> symbol (called the URL fragment). This part of the URL is <strong>never sent to our servers</strong>, so only you or anyone you share the full URL with can decrypt the Snipto.') !!}
                </p>
                <p class="mt-2">
                    {!! __('Each Snipto gets its <strong>own randomly generated encryption key</strong>, and the full URL—including this key—is shown to you immediately after creating the Snipto. Make sure to save or share it, because without full URL, the Snipto cannot be decrypted.') !!}
                </p>
            </div>
            ...
            <div id="lost-key" class="scroll-mt-13">
                ...
                <p>
                    {!! __('If you lose the key (the <code class="bg-gray-100 dark:bg-gray-700 px-1 rounded">#</code> part of the URL), the Snipto cannot be decrypted. Since we never store or know your key, we cannot help you recover it. <strong>No key, no decryption.</strong>') !!}
                </p>
            </div>
            ...
            <div id="personal-info" class="scroll-mt-13">
                ...
                <p>
                    {!! __('No personal identifiable information is stored or collected with your Snipto. We only store the <strong>ciphertext</strong> (your Snipto in encrypted form), the <strong>initialization vector (IV)</strong>, and metadata about expiration and whether the Snipto has been viewed. Your IP address or any other identifying info is <strong>never stored</strong> with your Snipto.') !!}
                </p>
            </div>
            ...
            <div id="end-to-end" class="scroll-mt-13">
                ...
                <p class="mt-2">
                    {!! __('All encryption and decryption happens <strong>directly in your browser</strong>. The JavaScript codet that performs these operations (snipto.js) is not minified and fully readable for transparency.') !!}
                </p>
                <p class="mt-2">
            ...
            <div id="iv" class="scroll-mt-12">
                <h2 class="text-xl font-semibold mb-2 text-indigo-600 dark:text-indigo-400">{!! __('What’s an initialization vector (IV)?') !!}</h2>
                <p class="mt-2">
                    {!! __('When your Snipto is encrypted, we don’t just scramble the text in the same way every time. To make it extra secure, a random “starting point” called an initialization vector (IV) is used. Think of it like adding a pinch of unique spice to each batch of cookies — even if the recipe is the same, each batch turns out slightly different.') !!}
                </p>
                <p class="mt-2">
                    {!! __('The IV ensures that two Sniptos with the same content don’t produce the same encrypted output. It’s stored alongside the encrypted Snipto so your browser can use it to decrypt the text, but it doesn’t compromise security — only someone with the encryption key can read the content.') !!}
                </p>
            </div>
        </div>
